...
function LineChart({
  ...
  return (
    ...
    >
      ...
      {/* 활성 구간 세로선 */}
      <line
        x1={toX(activeTierIndex)}
        y1={PT}
        x2={toX(activeTierIndex)}
        y2={PT + cH}
        stroke="#f3f4f6"
        strokeWidth={1.5}
        strokeDasharray="3 2"
...
export default function PhoneCompare(props) {
  ...
  const [slots, setSlots] = useState<(DeviceInfo | null)[]>([
    null,
    null,
    null,
  ])
  ...
  const fetchPrices = useCallback(
    async (models: string[], reg: RegisterType) => {
      if (models.length === 0) return
      setLoading(true)
      try {
        const res = await fetch(
          `${API}/api/compare/prices?models=${models.join(",")}&register=${reg}`
        )
  ...
  const savings =
    secondBestMonthly > 0 ? secondBestMonthly - winnerMonthly : 0
        ...
        <div style={{ margin: "0 16px 14px" }}>
          ...
          >
            ...
            <div
              style={{
                display: "grid",
                gridTemplateColumns: `80px ${slots
                  .filter(Boolean)
                  .map(() => "1fr")
                  .join(" ")}`,
                padding: "9px 14px",
